...
| **Form Requests** | 0% Coverage | Controllers rely on `$request->validate([...])` directly inside action methods. |
...
1. **Extract Form Requests**: Move `$request->validate()` rules into dedicated FormRequest classes (e.g. `PosAddItemRequest`, `CheckoutRequest`, `PlaceQrOrderRequest`).
