...
- **Key Deliverables**:
  ...
  - ✅ `StockService` with row-level `lockForUpdate` on deduct/restore.
  - ✅ Database transaction wrappers around checkout and order placement.
  - ✅ Composite performance indexes on `sales`, `sale_items`, `products`, and `dining_tables`.
...
- **Key Deliverables**:
  ...
  - ✅ Real-time customer order tracker view (`/menu/tracker/{token}` + `/order-status/{order}`).
...
- **Key Deliverables**:
  - ✅ PWA manifest with 192/512 icons, offline fallback, and Service Worker app-shell/runtime caching.
  ...
  - ✅ Server-side idempotent reconciliation (`POST /api/seller/pos/offline-sync`) with server-derived prices, stock, table, sale, and KOT integration.
