...
class ProductModifierController extends Controller
{
    public function index(Product $product)
    {
        ...
        return view('seller.products.modifiers', compact('product', 'availableModifiers', 'allModifiers'));
    }
    ...
    public function store(Request $request, Product $product)
    {
        ...
        if ($request->filled('modifier_id')) {
            $data = $request->validate([
                'modifier_id' => [
                    'required',
                    Rule::exists('modifiers', 'id')->where(fn ($q) => $q->where('seller_id', Auth::id())),
                ],
            ...
            return redirect()
                ->route('seller.products.modifiers.index', $product)
