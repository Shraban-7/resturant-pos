    ...
    <div class="relative" x-data="notifBell()" @keydown.escape.window="open = false" @notif-live.window="await refresh(false)">
        ...
        <div x-show="open" @click.outside="open = false" x-transition class="dropdown-menu !p-0 w-[22rem] max-w-[90vw]" style="display:none">
            <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                <p class="font-semibold text-sm text-slate-900">{{ __('admin.navbar.notifications') }}</p>
                <button type="button" @click="readAll()" class="text-xs font-semibold text-brand-600 hover:underline">{{ __('admin.navbar.mark_all_read') }}</button>
            </div>
            <div class="max-h-[22rem] overflow-y-auto" id="notif-list">
                ...
                <div x-show="!items.length" class="px-4 py-8 text-center text-slate-500">
                    ...
                    <p class="text-sm font-medium mt-1">{{ __('admin.navbar.no_notifications') }}</p>
                </div>
            </div>
            <a href="{{ route('admin.notifications.index') }}" class="block text-center text-xs font-semibold text-slate-600 hover:text-brand-600 py-2.5 border-t border-slate-100">{{ __('admin.navbar.view_all') }}</a>
        </div>
    </div>
    ...
    <div class="relative" x-data="{ open: false }" @keydown.escape.window="open = false">
        <button type="button" class="flex items-center gap-2 pl-1 pr-2 py-1 rounded-full hover:bg-slate-100 transition" @click="open = !open" :aria-expanded="open">
            ...
            <span class="hidden md:flex flex-col items-start leading-tight">
                ...
                <span class="text-[10px] uppercase tracking-wider text-slate-400">{{ is_employee() ? __('admin.navbar.employee') : __('admin.navbar.admin') }}</span>
            </span>
...
<i class="ri-settings-3-line me-2"></i> {{ __('admin.navbar.settings') }}
            </a>
            <div class="border-t border-slate-100 my-1"></div>
            <a href="{{ route('logout') }}" class="dropdown-item text-red-600">
                <i class="ri-logout-box-r-line me-2"></i> {{ __('admin.navbar.logout') }}
            </a>
        </div>
    </div>
</div>
