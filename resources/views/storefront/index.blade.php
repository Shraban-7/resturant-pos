                    ...
                    <div x-show="'{{ strtolower($product->name) }}'.includes(query.toLowerCase()) && (slot === 'all' || {{ (!$product->meal_times || $product->meal_times->isEmpty()) ? 'true' : 'false' }} || @js($product->meal_times?->map(fn ($m) => $m instanceof \App\Enums\MealSlot ? $m->value : (string) $m)->all() ?? []).includes(slot))"
                        ...
                        <div class="p-3">
                            ...
                            <p class="mt-1 flex flex-wrap gap-1">
                                ...
                                @else
                                    @foreach ($product->meal_times as $slot)
                                        <span class="text-[10px] font-semibold text-amber-700 bg-amber-50 rounded-full px-2 py-0.5">{{ $slot instanceof \App\Enums\MealSlot ? $slot->label() : ucfirst((string) $slot) }}</span>
                                    @endforeach
                                @endif
                            </p>
                            @php $sfProdType = $product->type instanceof \App\Enums\ProductType ? $product->type : \App\Enums\ProductType::tryFrom((string) $product->type); @endphp
                            ...
                            <div class="flex items-center justify-between mt-1.5">
                                <p class="font-bold text-orange-700 text-sm">{{ money($product->selling_price) }}@if ($sfProdType === \App\Enums\ProductType::BUFFET)<span class="font-medium text-slate-500">/person</span>@endif</p>
