                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <h6 class="card-title">{{ currency() }} {{ $product->price }}</h6>
                                    </div>
                                </div>
                            </div>
                            <div class="d-grid gap-2">
                                <a href="{{ route('seller.add_to_cart', $product->id) }}" class="btn btn-outline-dark btn-sm btn-block">Add</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if($sale)
                            ...
                            <div class="d-flex justify-content-start">
                                ...
                                <div class="mx-2 w-80">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <p class="fw-bold m-0">{{ currency() }} {{ $item->total_price }}</p>
                                    </div>
                                    <div>
                                        <div class="input-group mt-1" >
                                            <a class="btn btn-outline-danger btn-sm" href="{{ route('seller.update_cart', $item->id) }}?type=decrement">
                                                <i class="bi bi-trash"></i>
                                            </a>
                                            <input type="text" class="form-control form-control-sm" value="{{ $item->quantity }}">
                                            <a class="btn btn-outline-success btn-sm" href="{{ route('seller.update_cart', $item->id) }}?type=increment">
                ...
                <div class="p-3 mt-2 rounded border">
                    <div class="d-flex justify-content-between">
                        ...
                        <span class="fw-bold">{{ currency() }} <span id="subtotal">{{ $sale->subtotal }}</span></span>
                    </div>
                    <form action="{{ route('seller.chekout', $sale->id) }}" target="_blank" method="POST">
                        @CSRF
                        <div class="mb-3">
