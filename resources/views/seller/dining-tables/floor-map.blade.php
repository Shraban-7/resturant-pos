...
<a href="{{ route('seller.floors.index') }}">Floors</a>
<span class="separator">/</span>
...
<div class="page-header">
    ...
    <div class="page-actions">
        <a href="{{ route('seller.floors.index') }}" class="btn btn-secondary"><i class="ri-building-line"></i> Floors</a>
        <a href="{{ route('seller.diningTables.index') }}" class="btn btn-secondary"><i class="ri-reserved-line"></i> Tables</a>
...
<div class="flex flex-wrap gap-2 mb-4">
    @forelse($floors as $floor)
        <a href="{{ route('seller.diningTables.floorMap', ['floor_id' => $floor->id]) }}"
...
(function () {
    ...
    document.getElementById('saveFloorPlanBtn')?.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(@json(route('seller.diningTables.savePositions')), {
                method: 'POST',
                headers: {
            ...
            alert(err.message || 'Could not save floor plan');
        }
    });
})();
</script>
