...
<div class="flex flex-col h-screen bg-slate-50 overflow-hidden" x-data="posApp()" x-cloak>
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-4 pb-24 lg:pb-4">
            ...
            {{-- Recent Orders --}}
            @include('components.pos._order-chips', ['title' => 'Recent Orders', 'icon' => 'ri-receipt-2-line', 'sales' => $recentSales, 'routeName' => 'admin.sales.invoice'])

            {{-- Running Orders --}}
            @include('components.pos._order-chips', ['title' => 'Running Orders', 'icon' => 'ri-restart-line', 'sales' => $runningSales, 'routeName' => 'admin.pos.index', 'showTable' => true])
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.updateSale = function () {
            ...
            .then(({ ok, d }) => {
                if (!ok) { btn.innerHTML = original; btn.disabled = false; showError(d.message || 'Error'); return; }
                const url = "{{ $sale ? route('admin.sales.invoice', $sale->order_id) : '#' }}";
                window.open(url, 'Invoice', 'width=800,height=600,scrollbars=yes,resizable=yes');
