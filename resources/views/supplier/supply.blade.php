...
@php
    ...
    $categoryIcons = [
        'Grains & Rice' => 'ri-shopping-bag-4-fill',
        'Dairy & Eggs' => 'ri-bread-line',
        'Meat & Poultry' => 'ri-knife-line',
        'Seafood' => 'ri-bowl-line',
        'Spices & Seasonings' => 'ri-flask-line',
        'Oil & Sauces' => 'ri-bubble-chart-line',
        'Packaging Supplies' => 'ri-box-3-line',
        'Cleaning Supplies' => 'ri-brush-line',
        'Baking Supplies' => 'ri-cake-2-line',
        'Utensils & Tools' => 'ri-tools-line',
    ];
@endphp

<x-error-modal />

<div class="flex flex-col h-screen bg-slate-50 overflow-hidden" x-data="supplyApp()" x-cloak>

    {{-- =================== TOP BAR =================== --}}
    <header class="bg-white border-b border-slate-200 h-16 flex items-center gap-3 px-4 shrink-0 z-20">
        <a href="{{ route('supplier.dashboard') }}" class="flex items-center gap-2.5 shrink-0">
            ...
            <div class="hidden sm:block">
                <div class="text-sm font-semibold text-slate-900 leading-tight">Supply POS</div>
        ...
        <div class="flex items-center gap-1">
            ...
            <a href="{{ route('supplier.dashboard') }}" class="btn btn-ghost btn-icon" title="Dashboard">
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-4 pb-24 lg:pb-4">
            ...
            <x-pos._order-chips
                ...
                :sales="$recentSales"
                routeName="supplier.sales.invoice" />
            ...
            {{-- Products --}}
            <div>
                <h3 class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Products</h3>
    ...
    <div x-show="cartOpen" x-transition.opacity class="lg:hidden fixed inset-0 z-40" style="display:none" @keydown.escape.window="cartOpen = false">
        ...
        <aside class="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[90vh] flex flex-col shadow-2xl"
            ...
            <div class="flex items-center justify-between p-4 border-b border-slate-200">
                <h2 class="text-base font-semibold text-slate-900">Current Supply</h2>
    ...
    <div x-show="barcodeOpen" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display:none" @keydown.escape.window="barcodeOpen = false" @close-barcode.window="barcodeOpen = false">
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 p-6" @click.stop>
            ...
            <p class="mt-2 text-xs text-slate-500">Press Enter to add the matching product to the supply.</p>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateCartTotals() {
            let total = 0;
            let count = 0;
            document.querySelectorAll('#cart .cart-item').forEach(el => {
                const priceEl = el.querySelector('.price');
                if (priceEl) total += parseFloat(priceEl.textContent) || 0;
                const qtyEl = el.querySelector('.quantityInput');
        ...
        document.addEventListener('click', function (e) {
            ...
            const existingItem = document.querySelector(`#cart .cart-item[data-itemid="${id}"]`);
            if (existingItem) {
        ...
        window.addItem = function () {
            ...
            fetch("{{ route('supplier.supply.addItem') }}", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
                body: JSON.stringify({ order_id: orderId, product_id: id, quantity, unit_price: price, discount })
            })
        ...
        window.removeItem = function (cartItemId) {
            fetch("{{ route('supplier.supply.removeItem') }}", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
            ...
            .then(r => r.json())
            .then(d => {
                const el = document.querySelector('#cart-item-' + cartItemId);
                if (el) el.remove();
        ...
        document.addEventListener('click', function (e) {
            if (e.target.matches('.increment, .decrement')) {
                const isIncrement = e.target.matches('.increment');
                const wrap = e.target.parentElement;
                const input = wrap.querySelector('.quantityInput');
                ...
                updateCartQuantity(cartItem.dataset.id);
            }
        });
        ...
        function updateCartQuantity(cartItemId) {
            ...
            fetch("{{ route('supplier.supply.updateQuantity') }}", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
        ...
        window.checkout = function () {
            ...
            fetch("{{ route('supplier.supply.checkout') }}", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
                body: JSON.stringify({
                    order_id: orderId,
            ...
            .then(({ ok, d }) => {
                ...
                const url = "{{ route('supplier.sales.invoice', $cart->order_id) }}";
